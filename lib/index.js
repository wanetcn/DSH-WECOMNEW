 ...
 */
export const inject = ['agentDefaultModel', 'agents', 'sessions', 'tools', 'attachments', 'loader'];
...
export const Config = z.object({
  ...
  files: z
    .object({
      ...
      /**
       * Master switch for the wecom_send_file tool (agent → WeCom file
       * handoff). Sends are restricted to the chatting user's own workspace
       * directory and the public workspace directory.
       */
      sendEnabled: z.boolean().default(true),
      /** Max outbound file bytes (gateway caps a file at ~50MB). */
      maxSendBytes: z.number().default(48 * 1024 * 1024)
    })
    .default({}),

  /**
   * Live progress bubble: one WeCom stream message created immediately on
   * each inbound message, refreshed with tool activity while the agent
   * works, and sealed with the final answer. Gateway caps a stream at 10
   * minutes; longer tasks fall back to proactive messages.
   */
  progress: z
    .object({
      /** Master switch for the streaming progress bubble. */
      enabled: z.boolean().default(true)
    })
    .default({}),
  ...
  security: z
    .object({
      ...
      /**
       * Root directory holding per-group-chat shared workspaces. Defaults
       * to a `groups/` directory next to usersRoot. Every group session's
       * cwd is `<groupsRoot>/<chatId>` — members share it, and the session
       * runs at a fixed workspace-write / no-approval level regardless of
       * who speaks first.
       */
      groupsRoot: z.string().default(''),
      /**
       * Department document directories: per-department shared folders with
       * an explicit member list. Members' sessions are told they may read
       * (and wecom_send_file may attach from) exactly these directories;
       * non-members are denied by the boundary prompt. Independent of the
       * private workspaces and the public workspace.
       */
      departments: z
        .array(
          z.object({
            /** Display name shown in prompts/status (e.g. 部门A文档). */
            name: z.string(),
            /** Absolute path of the department directory. */
            dir: z.string(),
            /** WeCom userids allowed to access this department directory. */
      ...
      /**
       * Outbound-file approval gate: non-admin wecom_send_file requests are
       * held until an admin replies 允许/拒绝 in the admin's own WeCom chat
       * (fail closed: admin offline or busy → refused; timeout → refused).
       */
      sendApproval: z.boolean().default(true),
      /** How long an outbound-file approval may stay pending (ms). */
      sendApprovalTimeoutMs: z.number().default(10 * 60_000),
      /** Files above this size always need approval, regardless of type (bytes). */
      sendApprovalMaxDirectBytes: z.number().default(5 * 1024 * 1024)
    })
    .default({}),
  ...
  /**
   * User-level scheduled prompts (/cron add ...). Jobs live in the plugin's
   * own state file — the system cron is never touched. A due job runs its
   * task as a normal turn in the creating chat and delivers the answer there.
   */
  cron: z.object({
    /** Master switch for the scheduler. */
    enabled: z.boolean().default(true),
    /** How often due jobs are checked (ms). */
