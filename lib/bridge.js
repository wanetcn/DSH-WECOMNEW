...
export class Bridge {
  ...
  async requestSendApproval(chat, realPath, size, args, signal) {
    ...
    // Fan out to EVERY configured admin who has an active chat: first
    // decision wins, remaining pending approvals are torn down.
    const adminIds = (security.adminIds || []).filter((id) => id !== chat.userId);
    if (adminIds.length === 0) return { allowed: true, reason: '' }; // only the requester is admin → own send
    ...
    const decision = await new Promise((resolve) => {
      const pendingByChat = new Map(); // chatKey -> chat with a pending approval
      const settle = (allowed, reason) => {
        clearTimeout(timer);
        signal?.removeEventListener('abort', onAbort);
        for (const [key, adminChat] of pendingByChat) {
      ...
      for (const adminId of adminIds) {
        const adminChat = this.chats.get(`wecom:${adminId}`);
        if (adminChat === undefined || adminChat.pendingApproval !== undefined) continue;
        adminChat.pendingApproval = { resolve: settleProxy };
        pendingByChat.set(`wecom:${adminId}`, adminChat);
        reached.push(adminId);
        this.deliverText(adminChat, requestText).catch(() => {});
      }
      if (pendingByChat.size === 0) {
        // No admin reachable (offline / busy): fail closed.
        settle(false, '管理员暂不在线或正在处理其他审批，文件未发送。');
        void this.deliverText(chat, '⚠️ 管理员暂不在线（无法发起审批），文件未发送。请稍后再试或联系管理员。').catch(() => {});
        return;
      }
      ...
      void this.deliverText(chat, `🕐 该文件包含代码/敏感内容，已提交给管理员（${reached.join('、')}）审批，批准后将自动发送。`).catch(() => {});
    });
    if (decision.allowed) {
