...
export class Bridge {
  ...
  async handleSendFileRequest(agent, args, signal) {
    ...
    try {
      ...
      // Outbound-file approval gate: non-admin sends wait for an admin's
      // decision in the admin's own WeCom chat before anything leaves.
      if (!chat.isAdmin && this.isSendApprovalRequired()) {
