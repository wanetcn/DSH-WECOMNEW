...
const COMMANDS = ['help', 'todo', 'history', 'switch', 'compress', 'stop', 'cron', 'reset', 'status'];
...
export class Bridge {
  ...
  async handleMessage(input) {
    const { chatId, userId, text, file, images, reply } = input;
    const hasText = typeof text === 'string' && text.trim() !== '';
    const hasImages = Array.isArray(images) && images.length > 0;
    if (!hasText && !file && !hasImages) return;
    ...
    // 纯文件/纯图片消息（无文字）：保存后把存储路径作为回答，不额外开
    // agent 会话，方便用户把路径复述给智能体做后续对话。
    if (file && !hasText) {
      await this.receiveFileOnly(userId, file, reply);
      return;
    }
    if (hasImages && !hasText) {
      await this.receiveImagesOnly(userId, images, reply);
      return;
    }
  ...
  /**
   * Save bare image messages (no caption) and reply with their storage
   * paths, mirroring the pure-file flow. No agent turn is started.
   */
  async receiveImagesOnly(userId, images, reply) {
    ...
    try {
      ...
      for (const [index, image] of images.entries()) {
        try {
          const saved = await this.saveInboundImage(usersRoot, userId, image, index);
          lines.push(`🖼️ 图片已保存到：${saved.path}`);
        } catch (error) {
          const why = error instanceof Error ? error.message : String(error);
          lines.push(`⚠️ 第 ${index + 1} 张图片接收失败：${why}`);
        }
      }
      await reply(`${lines.join('\n')}\n可引用上面的路径向我提问，例如「分析这张截图」。`);
    } catch (error) {
      const why = error instanceof Error ? error.message : String(error);
      try {
        await reply(`❌ 图片接收失败: ${why}`);
      } catch {
        /* containment */
      }
    }
  ...
  async handleCronCommand(chatId, rest, input) {
    ...
    if (sub === 'add') {
      ...
      await input.reply(`✅ 定时任务已创建：\n- 名称：${name}\n- 计划：${describeCronSchedule(job)}\n- 任务：${task}\n- 下次执行：${cronTimeString(job.nextRunAt)}`);
      return;
    }
    ...
    if (sub === 'list') {
      ...
      for (const job of jobs) {
        lines.push(`- [${job.enabled ? '✅' : '⏸'}] ${job.name}（id:${job.id}）${describeCronSchedule(job)}，下次执行：${job.enabled ? cronTimeString(job.nextRunAt) : '—'}`);
      }
   ...
   * chat's model selection, preset and boundary prompt. The caller owns
   * error handling.
   */
  ...
  async createChat(key, userId, isGroupChat = false) {
    ...
    if (isGroupChat) {
      ...
      boundaryText = isAdmin ? adminBoundaryPrompt(cwd) : userBoundaryPrompt(cwd, publicDir);
    }
    await this.ensureWorkspace(cwd);
  ...
  async handleSendFileRequest(agent, args, signal) {
    ...
    try {
      ...
      // group chats — or the public workspace. realpath first, so symlink
      // and `..` traversal cannot escape the allow-list.
      const roots = [String(chat.cwd).replace(/\/+$/, ''), String(chat.publicDir).replace(/\/+$/, '')];
      let real;
      try {
