...
export class Bridge {
  ...
  async handleNameCommand(chatId, rest, input) {
    ...
    const live = this.ctx.agents.get(SessionId(targetSessionId));
    if (live === undefined) {
      await input.reply(`⚠️ ${label}未加载，请先 /switch 到它再命名（或直接用 /switch 切换后重新 /name）。`);
      return;
    }
    const sessionTitle = this.ctx.get('sessionTitle');
    if (sessionTitle === undefined) {
      await input.reply('⚠️ 命名服务不可用。');
      return;
    }
    try {
      sessionTitle.rename(live.session, title);
