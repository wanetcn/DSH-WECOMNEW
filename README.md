...
- **文件收发**：企业微信发来的文件自动下载、AES 解密并落盘到用户工作区，路径回传给会话与用户
- **多用户隔离**：每个企业微信用户独立私有工作区（会话 cwd 即写边界），管理员可获完整沙箱权限
- **Web 状态面板**：dsh web 界面的会话头部出现 💬 图标，点击查看桥接连接状态、在线时长、活动会话、历史总会话、收发消息计数与待用户应答数（数据来自插件注册的 `/api/wecom/status` 路由，计数器持久化、重启不丢）
